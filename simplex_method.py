...
Use maxz() to solve a maximization LP problem. Use minimize() to solve a minimization problem.
Disclosure -- pivot() function, subcomponent of maxz() and minimize(), has a couple bugs. So far, these have only occurred when
    minimize() has been called.
"""
...
# adds the objective function into the matrix.
def obj(table, eq):
    if add_obj(table):
...
def maximize(table, output='summary'):
...
def minimize(table, output='summary'):
